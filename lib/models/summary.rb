class Pdf::Summary < Prawn::Document
  ...
  def bbalance_row(m)
    tca = @ledger.balances[:savings][m][:bbalance] + @ledger.balances[:checking][m][:bbalance]
    cntnt = " #{m} Beginning Balance (Current Assets #{money(tca)} )"
    arr = [{content: cntnt,font_style: :bold}]
    arr << {content: money(@ledger.balances[:checking][m][:bbalance]),colspan:2, align: :center,font_style: :bold}
    @ledger.checking_funds.each do |f|
      arr << { content: money(@ledger.balances[f][m][:bbalance]),colspan:2, align: :center,font_style: :bold}
    end
    arr << {content: money(@ledger.balances[:savings][m][:bbalance]),colspan:2, align: :center,font_style: :bold}
    arr
  end
  ...
  def ebalance_row(m)
    ...
    arr = [{content: cntnt,font_style: :bold}]
    arr << {content: money(@ledger.balances[:checking][m][:ebalance]),colspan:2, align: :center,font_style: :bold}
    @ledger.checking_funds.each do |f|
      arr << { content: money(@ledger.balances[f][m][:ebalance]),colspan:2, align: :center,font_style: :bold}
    end
    arr << {content: money(@ledger.balances[:savings][m][:ebalance]),colspan:2, align: :center,font_style: :bold}
    arr
  end

  def summary_row(m)
    pl = @ledger.balances[:savings][m][:diff] + @ledger.balances[:checking][m][:diff]
  ...
  def build_table
    @rows = [header]
    @ledger.tmonths.each do |m|
      @rows << bbalance_row(m)
      @rows << summary_row(m)
    end
    m = @ledger.tmonths.last
    @rows << ebalance_row(m)
  end
